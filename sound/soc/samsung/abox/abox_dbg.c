...
static int __init abox_rmem_setup(unsigned long node,
...
static const struct reserved_mem_ops abox_rmem_ops = {
	.node_init = abox_rmem_setup,
};

RESERVEDMEM_OF_DECLARE(abox_rmem, "exynos,abox_rmem", &abox_rmem_ops);
		...
		struct abox_dbg_dump_min *p_dump)
{
	...
	return vm_map_ram(p_dump->pages, npages, -1);
...
static int samsung_abox_debug_probe(struct platform_device *pdev)
{
	...
	if (abox_rmem) {
		...
		abox_iommu_map_legacy(data->iommu_domain, IOVA_DUMP_BUFFER, abox_rmem->base,
				abox_rmem->size, 0);
	}
...
static void samsung_abox_debug_remove(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
	int i;
