...
#endif
	ret = abox_iommu_map_legacy(platform_data->abox_data->iommu_domain,
		...
		struct snd_compr_stream *stream)
{
	...
	abox_request_cpu_gear_dai(dev, abox_data, snd_soc_rtd_to_cpu(rtd, 0),
...
#else
{
	...
	abox_sysmmu_tlb_invalidate(abox_data->iommu_domain,
		...
		struct snd_compr_tstamp64 *tstamp)
{
	...
	if (pcm_size) {
		...
		dev_dbg(dev, "%s: pcm_size(%u), frame_count(%llu), copied_total(%llu)\n",
		...
		struct snd_compr_stream *stream, struct vm_area_struct *vma)
{
	...
	return abox_dma_mmap_writecombine(dev, vma,
			runtime->buffer,
...
static int abox_compr_write_data(struct snd_soc_component *component,
		struct snd_compr_stream *stream, const char __user *buf,
		size_t count)
{
	void *dstn;
	size_t copy;
...
static int abox_rdma_compr_copy(struct snd_soc_component *component,
		struct snd_compr_stream *stream, char __user *buf, size_t count)
{
	struct device *dev = component->dev;
	struct abox_platform_data *platform_data = dev_get_drvdata(dev);
	int id = platform_data->id;
...
static int abox_rdma_compr_get_codec_caps(
...
static const struct snd_compress_ops abox_rdma_compr_ops = {
		...
		struct snd_pcm_hw_params *params)
{
	...
	if (data->buf_type == BUFFER_TYPE_DMA) {
		...
		ret = abox_iommu_map_legacy(data->abox_data->iommu_domain, IOVA_RDMA_BUFFER(id),
	...
	abox_request_lit_freq_dai(dev, data->abox_data, snd_soc_rtd_to_cpu(rtd, 0), lit);
	abox_request_big_freq_dai(dev, data->abox_data, snd_soc_rtd_to_cpu(rtd, 0), big);
	abox_request_hmp_boost_dai(dev, data->abox_data, snd_soc_rtd_to_cpu(rtd, 0), hmp);
		...
		struct snd_pcm_substream *substream)
{
	...
	if (data->buf_type == BUFFER_TYPE_DMA) {
		iommu_unmap(data->abox_data->iommu_domain, IOVA_RDMA_BUFFER(id),
					...
					PAGE_SIZE));
		abox_sysmmu_tlb_invalidate(data->abox_data->iommu_domain,
					(dma_addr_t)IOVA_RDMA_BUFFER(id),
					round_up(substream->runtime->dma_bytes,
					PAGE_SIZE));
	}
#endif
	abox_request_lit_freq_dai(dev, data->abox_data, snd_soc_rtd_to_cpu(rtd, 0), 0);
	abox_request_big_freq_dai(dev, data->abox_data, snd_soc_rtd_to_cpu(rtd, 0), 0);
	abox_request_hmp_boost_dai(dev, data->abox_data, snd_soc_rtd_to_cpu(rtd, 0), 0);
...
static snd_pcm_uframes_t abox_rdma_pointer(
		struct snd_soc_component *component,
		struct snd_pcm_substream *substream)
{
	struct device *dev = component->dev;
		...
		struct snd_pcm_substream *substream, struct vm_area_struct *vma)
{
	 ...
	 */
	abox_request_cpu_gear_dai(dev, data->abox_data, snd_soc_rtd_to_cpu(rtd, 0),
...
static int abox_rdma_ack(struct snd_soc_component *component,
		struct snd_pcm_substream *substream)
{
	struct device *dev = component->dev;
	struct abox_platform_data *data = dev_get_drvdata(dev);
...
static int abox_rdma_ioctl(struct snd_soc_component *component,
...
static int abox_rdma_new(struct snd_soc_component *component,
		struct snd_soc_pcm_runtime *runtime)
{
	...
	if (data->buf_type == BUFFER_TYPE_ION) {
		...
		ret = abox_iommu_map_legacy(iommu_domain,
					    IOVA_RDMA_BUFFER(id),
...
static void abox_rdma_free(struct snd_soc_component *component,
		struct snd_pcm *pcm)
{
	struct device *dev = component->dev;
	struct abox_platform_data *data = dev_get_drvdata(dev);
	...
	if (data->buf_type == BUFFER_TYPE_ION) {
		...
		if (data->hwdep) {
			snd_device_free(data->hwdep->card, data->hwdep);
			data->hwdep = NULL;
		}
	} else {
#ifdef USE_FIXED_MEMORY
...
static int abox_rdma_probe(struct snd_soc_component *platform)
{
	struct device *dev = platform->dev;
	struct abox_platform_data *data =
	...
	if (data->type == PLATFORM_COMPRESS) {
		...
		ret = snd_soc_add_component_controls(platform,
...
static void abox_rdma_remove(struct snd_soc_component *platform)
{
	struct abox_platform_data *data =
			snd_soc_component_get_drvdata(platform);
...
static int samsung_abox_rdma_probe(struct platform_device *pdev)
{
	...
	ret = devm_snd_soc_register_component(dev, &abox_rdma, NULL, 0);
	if (ret < 0)
		return ret;
...
static void samsung_abox_rdma_remove(struct platform_device *pdev)
{
