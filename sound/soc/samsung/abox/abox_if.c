...
static int abox_dsif_set_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	...
	abox_component_read(cmpnt, ABOX_DSIF_CTRL, &ctrl);
...
static int abox_uaif_set_fmt(struct snd_soc_dai *dai, unsigned int fmt)
{
	...
	abox_component_read(cmpnt, ABOX_UAIF_CTRL0(id), &ctrl0);
	abox_component_read(cmpnt, ABOX_UAIF_CTRL1(id), &ctrl1);
...
static int abox_if_cmpnt_probe(struct snd_soc_component *cmpnt)
{
	...
	abox_register_if(data->abox_data->pdev, to_platform_device(dev),
			data->id, snd_soc_component_to_dapm(cmpnt),
			data->of_data->get_dai_name(data->id),
...
static const char *abox_dsif_get_str_name(int id, int stream)
{
...
static const char *abox_uaif_get_str_name(int id, int stream)
{
...
static void samsung_abox_if_remove(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
