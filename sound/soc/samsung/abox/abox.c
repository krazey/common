...
static void abox_iommu_domain_release(void *arg)
{
	struct abox_data *data = arg;
	...
	if (data->iommu_attached) {
		iommu_detach_device(data->iommu_domain, dev);
		data->iommu_attached = false;
	}
...
static noinline int abox_sif_format_put_ipc(struct device *dev, snd_pcm_format_t format,
		...
		struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	list_for_each_entry(dpcm, &be->dpcm[stream].fe_clients, list_fe) {
		if (snd_soc_rtd_to_cpu(dpcm->fe, 0)->id != ABOX_WDMA0) {
			skip = false;
			break;
		}
	}
...
static void abox_cmpnt_remove(struct snd_soc_component *component)
{
	...
	/*
	 * The platform device owns this devm-managed regmap.  A sound-card
	 * bind failure can remove the component while ABOX remains active.
	 */
	data->cmpnt = NULL;
}
...
static noinline int abox_sample_rate_put_ipc(struct device *dev, unsigned int val,
		enum ABOX_CONFIGMSG configmsg)
{
	struct abox_data *data = dev_get_drvdata(dev);
	ABOX_IPC_MSG msg;
...
static noinline int abox_bit_width_put_ipc(struct device *dev, unsigned int val,
		enum ABOX_CONFIGMSG configmsg)
{
	struct abox_data *data = dev_get_drvdata(dev);
	snd_pcm_format_t format = SNDRV_PCM_FORMAT_S16;
...
static noinline int abox_erap_handler_put_ipc(struct device *dev,
...
static noinline int abox_audio_mode_put_ipc(struct device *dev, enum audio_mode mode)
{
	struct abox_data *data = dev_get_drvdata(dev);
	ABOX_IPC_MSG msg;
...
static noinline int abox_sound_type_put_ipc(struct device *dev, enum sound_type type)
{
	struct abox_data *data = dev_get_drvdata(dev);
	ABOX_IPC_MSG msg;
	struct IPC_SYSTEM_MSG *system_msg = &msg.msg.system;
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	if (!abox_dapm_connected_input_ep(w, NULL)) {
		dev_info(dev, "%s: flush\n", __func__);
		...
		struct snd_kcontrol *kcontrol, int event)
{
	...
	if (!abox_dapm_connected_output_ep(w, NULL)) {
		dev_info(dev, "%s: flush\n", __func__);
		...
		struct snd_pcm_hw_params *params, int stream)
{
	...
	snd_soc_dapm_mutex_lock(snd_soc_component_to_dapm(cmpnt));
	...
	snd_soc_dapm_widget_for_each_source_path(snd_soc_dai_get_widget_playback(dai), path) {
		if (path->connect) {
			w = path->node[SND_SOC_DAPM_DIR_IN];
			abox_dapm_connected_input_ep(w, &widget_list);
		}
	}
	snd_soc_dapm_widget_for_each_sink_path(snd_soc_dai_get_widget_capture(dai), path) {
		if (path->connect) {
			w = path->node[SND_SOC_DAPM_DIR_OUT];
			abox_dapm_connected_output_ep(w, &widget_list);
		}
	}
	...
	if (abox_if_hw_params_fixup(rtd, params, stream) >= 0) {
		...
		if (stream_mst == SNDRV_PCM_STREAM_PLAYBACK)
			abox_dapm_connected_output_ep(w_mst, &widget_list);
		else
			abox_dapm_connected_input_ep(w_mst, &widget_list);
	...
	if (dai->driver->symmetric_sample_bits && dai->symmetric_sample_bits &&
			dai->symmetric_sample_bits != width) {
		width = dai->symmetric_sample_bits;
		abox_set_sif_width(data, msg_format, dai->symmetric_sample_bits);
		format = abox_get_sif_format(data, msg_format);
	}

	if (dai->driver->symmetric_channels && dai->symmetric_channels &&
			dai->symmetric_channels != channels)
		channels = dai->symmetric_channels;

	if (dai->driver->symmetric_rate && dai->symmetric_rate && dai->symmetric_rate != rate)
		rate = dai->symmetric_rate;
	...
	dev_dbg(dev, "%s: set to %u bit, %u channel, %uHz\n", __func__,
			width, channels, rate);
unlock:
	snd_soc_dapm_mutex_unlock(snd_soc_component_to_dapm(cmpnt));
...
static struct abox_qos_vote abox_qos_aud = { .name = "aud" };
static struct abox_qos_vote abox_qos_int = { .name = "int" };
static struct abox_qos_vote abox_qos_mif = { .name = "mif" };
static struct abox_qos_vote abox_qos_lit = { .name = "lit" };
static struct abox_qos_vote abox_qos_big = { .name = "big" };

static int abox_qos_notifier(struct notifier_block *nb,
		unsigned long action, void *nb_data);

static int abox_qos_vote_update(struct device *dev,
		struct abox_qos_vote *vote, unsigned int value)
{
	int ret = 0;
	...
	if (vote->clk)
		ret = clk_set_rate(vote->clk, (unsigned long)value * 1000);
	else if (vote->active)
		ret = freq_qos_update_request(&vote->freq_req, value);

	if (ret < 0) {
		dev_err(dev, "failed to vote %s frequency %u kHz: %d\n",
				vote->name, value, ret);
		return ret;
	}
...
static int abox_qos_votes_init(struct device *dev)
{
	struct clk *aud, *int_clk, *mif;
	int ret;

	aud = devm_clk_get_optional(dev, "dvfs-aud");
	if (IS_ERR(aud))
		return dev_err_probe(dev, PTR_ERR(aud),
				"failed to get AUD DVFS clock\n");

	int_clk = devm_clk_get_optional(dev, "dvfs-int");
	if (IS_ERR(int_clk))
		return dev_err_probe(dev, PTR_ERR(int_clk),
				"failed to get INT DVFS clock\n");

	mif = devm_clk_get_optional(dev, "dvfs-mif");
	if (IS_ERR(mif))
		return dev_err_probe(dev, PTR_ERR(mif),
				"failed to get MIF DVFS clock\n");

	abox_qos_aud.clk = aud;
	abox_qos_int.clk = int_clk;
	abox_qos_mif.clk = mif;

	abox_qos_lit.policy = cpufreq_cpu_get(0);
	if (abox_qos_lit.policy) {
		ret = freq_qos_add_request(&abox_qos_lit.policy->constraints,
				&abox_qos_lit.freq_req, FREQ_QOS_MIN, 0);
		if (ret < 0) {
			cpufreq_cpu_put(abox_qos_lit.policy);
			abox_qos_lit.policy = NULL;
		} else {
			abox_qos_lit.active = true;
		}
	}

	abox_qos_big.policy = cpufreq_cpu_get(4);
	if (abox_qos_big.policy) {
		ret = freq_qos_add_request(&abox_qos_big.policy->constraints,
				&abox_qos_big.freq_req, FREQ_QOS_MIN, 0);
		if (ret < 0) {
			cpufreq_cpu_put(abox_qos_big.policy);
			abox_qos_big.policy = NULL;
		} else {
			abox_qos_big.active = true;
		}
	}

	if (!abox_qos_lit.active)
		dev_warn(dev, "little-cluster QoS vote is unavailable\n");
	if (!abox_qos_big.active)
		dev_warn(dev, "big-cluster QoS vote is unavailable\n");

	return 0;
}

static void abox_qos_vote_exit(struct abox_qos_vote *vote)
{
	if (vote->active) {
		freq_qos_remove_request(&vote->freq_req);
		vote->active = false;
	}
	if (vote->policy) {
		cpufreq_cpu_put(vote->policy);
		vote->policy = NULL;
	}
	vote->clk = NULL;
	vote->value = 0;
...
static void abox_qos_votes_exit(void)
{
	abox_qos_vote_exit(&abox_qos_aud);
	abox_qos_vote_exit(&abox_qos_int);
	abox_qos_vote_exit(&abox_qos_mif);
	abox_qos_vote_exit(&abox_qos_lit);
	abox_qos_vote_exit(&abox_qos_big);
}

static void abox_qos_votes_exit_action(void *unused)
{
	abox_qos_votes_exit();
}
		...
		struct abox_data *data)
{
	...
	if (increasing) {
		if (gear <= ARRAY_SIZE(data->pm_qos_int))
			abox_qos_vote_update(dev, &abox_qos_int,
					data->pm_qos_int[gear - 1]);
		else
			abox_qos_vote_update(dev, &abox_qos_int, 0);
	}
...
static void abox_change_cpu_gear(struct device *dev, struct abox_data *data)
{
	...
	if (data->cpu_gear != gear)
		abox_qos_vote_update(dev, &abox_qos_aud, freq);

	dev_info(dev, "pm qos request aud: req=%dkHz ret=%dkHz\n", freq,
			abox_qos_aud.value);
	abox_qos_notifier(&data->qos_nb, abox_qos_aud.value, NULL);
	abox_notify_cpu_gear(data, abox_qos_aud.value * 1000);
...
static void abox_change_int_freq_work_func(struct work_struct *work)
{
	...
	if (data->int_freq != freq) {
		data->int_freq = freq;
		abox_qos_vote_update(dev, &abox_qos_int, data->int_freq);
...
static void abox_change_mif_freq_work_func(struct work_struct *work)
{
	...
	if (data->mif_freq != freq) {
		data->mif_freq = freq;
		abox_qos_vote_update(dev, &abox_qos_mif, data->mif_freq);
...
static void abox_change_lit_freq_work_func(struct work_struct *work)
{
	...
	if (data->lit_freq != freq) {
		data->lit_freq = freq;
		abox_qos_vote_update(dev, &abox_qos_lit, data->lit_freq);
...
static void abox_change_big_freq_work_func(struct work_struct *work)
{
	...
	if (data->big_freq != freq) {
		data->big_freq = freq;
		abox_qos_vote_update(dev, &abox_qos_big, data->big_freq);
		...
		phys_addr_t paddr, size_t size)
{
	...
	ret = abox_iommu_map_legacy(data->iommu_domain, iova, paddr, size, 0);
	if (ret < 0) {
		...
		int prot, size_t bytes, void *area)
{
	...
	if (ret != bytes) {
		dev_err(dev, "Short iommu_map_sg(%#lx): %d/%zu\n", iova,
				ret, bytes);
		return -EIO;
	}
		...
		phys_addr_t paddr, size_t size)
{
	...
	abox_sysmmu_tlb_invalidate(data->iommu_domain, iova, size);
...
{
	struct snd_soc_dai *dai = snd_soc_rtd_to_cpu(fe, 0);
	struct snd_soc_component *cmpnt = dai->component;
	...
	if (snd_soc_dai_get_widget_playback(dai))
		abox_dapm_connected_output_ep(snd_soc_dai_get_widget_playback(dai),
				&widget_list);
	if (snd_soc_dai_get_widget_capture(dai))
		abox_dapm_connected_input_ep(snd_soc_dai_get_widget_capture(dai),
				&widget_list);
		...
		bool playback, bool capture)
{
	...
	if (data->cmpnt->name_prefix &&
			snd_soc_dapm_to_component(dapm)->name_prefix &&
...
static noinline int abox_component_control_put_ipc(struct device *dev,
		struct abox_component_kcontrol_value *value)
{
	ABOX_IPC_MSG msg;
	struct IPC_SYSTEM_MSG *system_msg = &msg.msg.system;
...
static void abox_download_extra_firmware(struct abox_data *data)
{
	...
	for (ext_fw = data->firmware_extra; ext_fw - data->firmware_extra <
		...
		if (io_base)
			memcpy_toio(io_base + ext_fw->offset,
				    ext_fw->firmware->data, ext_fw->firmware->size);
		else
		...
		phys_addr_t *addr)
{
	...
	ret = abox_iommu_map_legacy(data->iommu_domain,
	...
	iommu_unmap(data->iommu_domain, IOVA_IVA(ABOX_IVA_MEMORY_PREPARE),
			IVA_FIRMWARE_SIZE);
	abox_sysmmu_tlb_invalidate(data->iommu_domain,
			IOVA_IVA(ABOX_IVA_MEMORY), IVA_FIRMWARE_SIZE);
	abox_sysmmu_tlb_invalidate(data->iommu_domain,
			IOVA_IVA(ABOX_IVA_MEMORY_PREPARE), IVA_FIRMWARE_SIZE);

	ret = abox_iommu_map_legacy(data->iommu_domain, IOVA_IVA(ABOX_IVA_MEMORY),
...
error:
	...
	abox_sysmmu_tlb_invalidate(data->iommu_domain,
			IOVA_IVA(ABOX_IVA_MEMORY_PREPARE), IVA_FIRMWARE_SIZE);
	abox_iommu_map_legacy(data->iommu_domain, IOVA_IVA(ABOX_IVA_MEMORY),
			data->iva_base_phys, IVA_FIRMWARE_SIZE, 0);
...
static int samsung_abox_probe(struct platform_device *pdev)
{
	...
	data->iommu_domain = iommu_paging_domain_alloc(dev);
	if (IS_ERR(data->iommu_domain)) {
		dev_err(dev, "Unable to get iommu domain\n");
		return PTR_ERR(data->iommu_domain);
	}
	ret = devm_add_action_or_reset(dev, abox_iommu_domain_release, data);
	if (ret < 0) {
		dev_err(dev, "Unable to register iommu cleanup (%d)\n", ret);
		return ret;
	}
	...
	if (iommu_get_domain_for_dev(dev) != data->iommu_domain) {
		iommu_detach_device(data->iommu_domain, dev);
		dev_err(dev, "ABOX iommu domain did not become active\n");
		return -EIO;
	}
...
#endif
	ret = abox_qos_votes_init(dev);
	if (ret < 0)
		return ret;
	ret = devm_add_action_or_reset(dev, abox_qos_votes_exit_action, NULL);
	if (ret < 0)
		return ret;
...
static void samsung_abox_remove(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
	struct abox_data *data = platform_get_drvdata(pdev);
