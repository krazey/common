...
static int abox_vdma_period_elapsed(struct abox_vdma_info *info,
		...
		struct snd_pcm_substream *substream)
{
	...
	struct abox_vdma_info *info = snd_soc_component_get_drvdata(component);
...
static snd_pcm_uframes_t abox_vdma_pointer(
...
static int abox_vdma_ack(struct snd_soc_component *component,
		struct snd_pcm_substream *substream)
{
...
static int abox_vdma_ioctl(struct snd_soc_component *component,
...
static int abox_vdma_platform_probe(
		struct snd_soc_component *platform)
{
	...
	snd_soc_component_set_drvdata(platform, abox_vdma_get_info(id));
	return 0;
}

static int abox_vdma_platform_new(struct snd_soc_component *component,
...
static void abox_vdma_platform_free(struct snd_soc_component *component,
		struct snd_pcm *pcm)
{
	struct device *dev = component->dev;
	struct device *dev_abox = abox_vdma_dev_abox;
...
static const struct snd_soc_component_driver abox_vdma_platform = {
	.name		= "samsung-abox-vdma",
	.probe		= abox_vdma_platform_probe,
	.open		= abox_vdma_open,
	.close		= abox_vdma_close,
	.ioctl		= abox_vdma_ioctl,
...
SND_SOC_DAILINK_DEF(abox_vdma_dummy,
		DAILINK_COMP_ARRAY(COMP_DUMMY()));

static void abox_vdma_register_work_func(struct work_struct *work)
{
	int id;
...
static unsigned int abox_vdma_rate_range_to_bits(unsigned int rate_min,
		unsigned int rate_max)
{
	static const unsigned int known_rates[] = {
		8000, 11025, 16000, 22050, 32000, 44100,
		48000, 64000, 88200, 96000, 176400, 192000,
		352800, 384000, 705600, 768000, 12000, 24000,
		128000,
	};
	...
	for (i = 0; i < ARRAY_SIZE(known_rates); i++) {
		if (known_rates[i] >= rate_min && known_rates[i] <= rate_max)
			rates |= snd_pcm_rate_to_rate_bit(known_rates[i]);
	}

	return rates ?: SNDRV_PCM_RATE_KNOT;
}
		...
		const struct PCMTASK_HARDWARE *pcm_hardware)
{
	...
	hardware->rates = (pcm_hardware->rate_max > 192000) ?
			SNDRV_PCM_RATE_KNOT : abox_vdma_rate_range_to_bits(
...
static void abox_vdma_register_card_work_func(struct work_struct *work)
{
	...
	for (i = 0; i < abox_vdma_card.num_links; i++) {
		...
		link->cpus = abox_vdma_dummy;
		link->num_cpus = ARRAY_SIZE(abox_vdma_dummy);
		link->codecs = abox_vdma_dummy;
		link->num_codecs = ARRAY_SIZE(abox_vdma_dummy);
		link->no_pcm = 1;
	}
...
static int abox_vdma_add_dai_link(struct device *dev)
{
	...
	if (idx < 0 || idx >= ARRAY_SIZE(abox_vdma_dai_links)) {
...
static void samsung_abox_vdma_remove(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
	int id = to_platform_device(dev)->id;
