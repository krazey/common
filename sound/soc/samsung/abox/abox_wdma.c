		...
		struct snd_pcm_hw_params *params)
{
	...
	if (data->buf_type == BUFFER_TYPE_DMA) {
		...
		ret = abox_iommu_map_legacy(abox_data->iommu_domain, IOVA_WDMA_BUFFER(id),
...
static snd_pcm_uframes_t abox_wdma_pointer(
...
static int abox_wdma_ack(struct snd_soc_component *component,
		struct snd_pcm_substream *substream)
{
	struct device *dev = component->dev;
	struct abox_platform_data *data = dev_get_drvdata(dev);
...
static int abox_wdma_ioctl(struct snd_soc_component *component,
...
static int abox_wdma_new(struct snd_soc_component *component,
...
static int samsung_abox_wdma_probe(struct platform_device *pdev)
{
	...
	ret = devm_snd_soc_register_component(dev, &abox_wdma, NULL, 0);
	if (ret < 0)
		return ret;
...
static void samsung_abox_wdma_remove(struct platform_device *pdev)
{
